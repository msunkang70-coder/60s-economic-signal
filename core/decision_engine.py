...
DECISION_TEMPLATES: dict[str, dict[str, dict[str, list[dict]]]] = {
    ...
    "일반": {
        ...
        "수출": {
            "danger": [
                {"title": "주요 수출 시장 긴급 점검", "rationale": "수출 급감 — 국가별 수요·재고 현황 즉시 파악", "urgency": "즉시", "difficulty": "낮음", "impact": "높음"},
                {"title": "생산·재고 계획 긴급 조정", "rationale": "수출 급감에 따른 재고 적체 방지 — 생산라인 조율", "urgency": "즉시", "difficulty": "중간", "impact": "높음"},
                {"title": "대체 시장 신속 탐색", "rationale": "주력 시장 수요 급감 시 신흥 시장 대안 점검", "urgency": "이번 주", "difficulty": "중간", "impact": "중간"},
            ],
            "warning": [
                {"title": "수출 감소 원인 분석", "rationale": "수요 둔화 vs 경쟁 심화 원인 구분 후 대응", "urgency": "이번 주", "difficulty": "낮음", "impact": "중간"},
                {"title": "신규 바이어 발굴 착수", "rationale": "수출 감소 선제 대응 — 신규 거래처 파이프라인 구축", "urgency": "이번 주", "difficulty": "중간", "impact": "높음"},
                {"title": "가격 경쟁력 재점검", "rationale": "경쟁사 대비 가격 포지션 재산정", "urgency": "이번 달", "difficulty": "중간", "impact": "중간"},
            ],
            "caution": [
                {"title": "수출 급증 지속 가능성 점검", "rationale": "15% 이상 급증 — 일시적인지 구조적 증가인지 분석", "urgency": "이번 주", "difficulty": "낮음", "impact": "중간"},
                {"title": "생산·공급 능력 검토", "rationale": "수출 급증 지속 시 대응 가능 생산 여력 확인", "urgency": "이번 달", "difficulty": "중간", "impact": "높음"},
                {"title": "신규 수출 계약 확대 추진", "rationale": "수출 호조 활용 — 장기 공급 계약 체결 추진", "urgency": "이번 달", "difficulty": "중간", "impact": "높음"},
            ],
            "normal": [
                {"title": "현행 수출 전략 유지", "rationale": "수출 안정 — 기존 계획 유지", "urgency": "이번 달", "difficulty": "낮음", "impact": "낮음"},
                {"title": "수출 시장 다변화 검토", "rationale": "안정기 활용 — 신규 국가·채널 탐색", "urgency": "이번 달", "difficulty": "중간", "impact": "중간"},
                {"title": "중장기 수출 역량 강화", "rationale": "안정적 환경에서 수출 경쟁력 제고 투자 계획", "urgency": "이번 달", "difficulty": "높음", "impact": "높음"},
            ],
        },
        "물가": {
            "danger": [
                {"title": "원자재 조달 비용 긴급 점검", "rationale": "고물가 — 핵심 원자재 비용 급등 영향 즉시 산출", "urgency": "즉시", "difficulty": "낮음", "impact": "높음"},
                {"title": "판매가 긴급 조정", "rationale": "원가 급등분 즉시 반영 — 마진 방어", "urgency": "즉시", "difficulty": "높음", "impact": "높음"},
                {"title": "대체 원자재·공급처 긴급 발굴", "rationale": "비용 급등 원자재 대체 옵션 확보로 리스크 분산", "urgency": "이번 주", "difficulty": "중간", "impact": "중간"},
            ],
            "warning": [
                {"title": "원자재 가격 주간 추적", "rationale": "물가 경고 — 핵심 원자재 비용 상승 사전 감지", "urgency": "이번 주", "difficulty": "낮음", "impact": "중간"},
                {"title": "단가 전가 가능성 분석", "rationale": "원가 상승분을 고객에 전가 가능 수준 분석", "urgency": "이번 주", "difficulty": "중간", "impact": "높음"},
                {"title": "고마진 제품 비중 확대", "rationale": "원가 압박 대비 고부가 제품 믹스 개선", "urgency": "이번 달", "difficulty": "중간", "impact": "높음"},
            ],
            "caution": [
                {"title": "월간 원가 구조 리뷰", "rationale": "주의 구간 — 주요 원자재 비용 월별 추적", "urgency": "이번 달", "difficulty": "낮음", "impact": "중간"},
                {"title": "장기 공급 계약 재검토", "rationale": "가격 변동 리스크 최소화 위한 고정가 조달 검토", "urgency": "이번 달", "difficulty": "중간", "impact": "중간"},
                {"title": "원가 절감 과제 발굴", "rationale": "물가 상승 추세 대비 원가 절감 우선순위 설정", "urgency": "이번 달", "difficulty": "중간", "impact": "높음"},
            ],
            "normal": [
                {"title": "현행 조달 전략 유지", "rationale": "물가 안정 — 기존 원자재 조달 계획 유지", "urgency": "이번 달", "difficulty": "낮음", "impact": "낮음"},
                {"title": "원가 절감 기회 탐색", "rationale": "안정기 활용 — 원자재·공정 최적화 검토", "urgency": "이번 달", "difficulty": "중간", "impact": "중간"},
                {"title": "설비 투자 타이밍 검토", "rationale": "원가 안정기 — 설비 효율화 투자 적기", "urgency": "이번 달", "difficulty": "높음", "impact": "높음"},
            ],
        },
        "금리": {
            "danger": [
                {"title": "차입금 금리 부담 긴급 점검", "rationale": "고금리 — 전체 차입금 이자 비용 급증 영향 즉시 산출", "urgency": "즉시", "difficulty": "낮음", "impact": "중간"},
                {"title": "신규 차입 계획 재검토", "rationale": "차입 비용 급증 — 신규 투자·운전자본 차입 계획 재조정", "urgency": "이번 주", "difficulty": "중간", "impact": "높음"},
                {"title": "고정금리 전환 긴급 추진", "rationale": "추가 금리 상승 리스크 차단 — 변동금리 즉시 고정", "urgency": "이번 주", "difficulty": "중간", "impact": "중간"},
            ],
            "warning": [
                {"title": "차입 비용 영향 분석", "rationale": "금리 경고 — 차입 이자 증가분 사업 수익성에 반영", "urgency": "이번 주", "difficulty": "낮음", "impact": "중간"},
                {"title": "투자 우선순위 재조정", "rationale": "ROI 낮은 투자 과제 연기 검토", "urgency": "이번 달", "difficulty": "중간", "impact": "중간"},
                {"title": "정책자금 활용 최대화", "rationale": "고금리 대비 정부 지원 자금 우선 활용", "urgency": "이번 달", "difficulty": "낮음", "impact": "중간"},
            ],
            "caution": [
                {"title": "차입 구조 최적화 점검", "rationale": "주의 구간 — 단기·장기 차입 비율 및 금리 조건 점검", "urgency": "이번 달", "difficulty": "낮음", "impact": "중간"},
                {"title": "투자 타이밍 재검토", "rationale": "금리 방향성 분석 후 투자 집행 타이밍 결정", "urgency": "이번 달", "difficulty": "중간", "impact": "중간"},
                {"title": "현금흐름 관리 강화", "rationale": "금리 변동기 유동성 버퍼 확보 전략 점검", "urgency": "이번 달", "difficulty": "낮음", "impact": "중간"},
            ],
            "normal": [
                {"title": "현행 금융 전략 유지", "rationale": "금리 안정 — 기존 차입·투자 계획 유지", "urgency": "이번 달", "difficulty": "낮음", "impact": "낮음"},
                {"title": "장기 투자 계획 구체화", "rationale": "안정적 금리 환경 활용 — 투자 실행 계획 수립", "urgency": "이번 달", "difficulty": "높음", "impact": "높음"},
                {"title": "자금 조달 다변화 검토", "rationale": "안정기 활용 — 회사채·정책자금 조달 채널 확보", "urgency": "이번 달", "difficulty": "중간", "impact": "중간"},
            ],
        },
    },
